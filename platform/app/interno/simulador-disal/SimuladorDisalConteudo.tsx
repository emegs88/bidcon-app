"use client";
// Conteúdo do /interno/simulador-disal — Simulador de planos novos Disal.
//
// Vive FORA do page.tsx porque um page.tsx do App Router só pode exportar o
// default (e um punhado de nomes reservados); qualquer export nomeado extra
// quebra o type-check do Next. Aqui podemos exportar o componente e os
// construtores para o teste-fumaça de render (lib/simulador-disal.test.tsx).
// Não é rota: sem page/layout/route no nome, o Next ignora no roteamento —
// mesmo padrão do SimuladorTabNav.tsx.
...
// ---------------------------------------------------------------------------
// Construtores de item — no escopo do MÓDULO de propósito: assim o estado
// inicial pode ser montado antes do primeiro render (o seed do teste-fumaça)
// e as funções ficam exercitáveis sem montar a tela inteira.
// ---------------------------------------------------------------------------
const { autosFaixaII: AUTOS_II, autosFaixaIII: AUTOS_III, imoveis220: IMOVEIS } = BOLETIM_DISAL_ATUAL;

function construirItemAuto(credito: number, quantidade: number, b: Base): ItemCarteiraUI {
  const { linha, faixa, rotuloFaixa } = linhaAutoMaisProxima(credito, AUTOS_II, AUTOS_III);
  const [creditoReal, cod, p100, p75] = linha;
  return {
    id: `veiculo|${cod}|${creditoReal}|${b}`,
    tipo: "veiculo",
    rotulo: `Veículo · ${fmtCredito(creditoReal)} · ${rotuloFaixa}`,
    cod,
    credito: creditoReal,
    quantidade,
    base: b,
...
function construirItemImovel(linha: LinhaImovel, quantidade: number, b: Base): ItemCarteiraUI {
  ...
  return {
    ...
    prazo: IMOVEIS.prazo,
    taxa: IMOVEIS.taxa,
    indice: IMOVEIS.indice,
    fases: FASES_IMOVEL_MESES.map((meses, i) => ({ meses, valor: f[i] })),
    fasesBase100: FASES_IMOVEL_MESES.map((meses, i) => ({ meses, valor: linha.b100[i] })),
  };
}

/** Imóvel pelo crédito, encaixando no valor mais próximo que EXISTE no boletim. */
export function construirItemImovelPorCredito(credito: number, quantidade: number, b: Base): ItemCarteiraUI {
  const linha = IMOVEIS.linhas.reduce((melhor, l) =>
    Math.abs(l.credito - credito) < Math.abs(melhor.credito - credito) ? l : melhor,
  );
  return construirItemImovel(linha, quantidade, b);
}

/** Estado inicial injetável — existe para o teste-fumaça poder renderizar a
 *  página com carteira JÁ preenchida. Em produção nada passa este prop. */
export type EstadoInicialSimulador = {
  cartas?: { tipo: Segmento; credito: number; quantidade: number }[];
  modo?: ModoCarteira;
  base?: Base;
  lancePct?: number;
  cJuncao?: number | null;
...
function construirItensIniciais(e?: EstadoInicialSimulador): ItemCarteiraUI[] {
  const b: Base = e?.base ?? "100";
  return (e?.cartas ?? []).map((c) =>
    c.tipo === "veiculo"
      ? construirItemAuto(c.credito, c.quantidade, b)
      : construirItemImovelPorCredito(c.credito, c.quantidade, b),
  );
}

export function SimuladorDisalConteudo({ estadoInicial }: { estadoInicial?: EstadoInicialSimulador }) {
  const { autosFaixaII, autosFaixaIII, imoveis220, mes } = BOLETIM_DISAL_ATUAL;

  const [segmento, setSegmento] = useState<Segmento>(estadoInicial?.cartas?.[0]?.tipo ?? "veiculo");
  const [base, setBase] = useState<Base>(estadoInicial?.base ?? "100");
  ...
  const [itens, setItens] = useState<ItemCarteiraUI[]>(() => construirItensIniciais(estadoInicial));
  const [modo, setModo] = useState<ModoCarteira>(estadoInicial?.modo ?? "juncao");
  // null = acompanha o default do segmento; número = cenário escolhido/digitado
  const [cJuncao, setCJuncao] = useState<number | null>(estadoInicial?.cJuncao ?? null);
  const [lancePct, setLancePct] = useState(estadoInicial?.lancePct ?? 0);
  const [parcelaAlvo, setParcelaAlvo] = useState("");
  const [frase, setFrase] = useState("");
  const [interpretando, setInterpretando] = useState(false);
  const [resumoPedido, setResumoPedido] = useState("");
  const [erroFrase, setErroFrase] = useState("");
  const [assistenteIndisponivel, setAssistenteIndisponivel] = useState(false);
  ...
  // wrappers finos sobre os construtores de módulo (default = base da tela)
  const montarItemAuto = (credito: number, quantidade: number, b: Base = base) =>
    construirItemAuto(credito, quantidade, b);
  const montarItemImovel = (linha: LinhaImovel, quantidade: number, b: Base = base) =>
    construirItemImovel(linha, quantidade, b);
  ...
  // ---------------------------------------------------------------------
  // ORDEM IMPORTA: `projecaoDisponivel` é lida por resumirCenario(), que é
  // CHAMADA logo abaixo no corpo do componente. Declarar isto depois criava
  // TDZ ("Cannot access ... before initialization") no primeiro render com
  // carteira não vazia — o crash ao adicionar a primeira carta. tsc não pega
  // esse tipo de erro; o teste-fumaça de render pega (lib/simulador-disal.test.tsx).
  ...
  async function interpretarFrase() {
    ...
    setErroFrase("");
    setResumoPedido("");
    setAssistenteIndisponivel(false);
    try {
      ...
      // Degradação: o assistente é um ATALHO, nunca um pré-requisito. Qualquer
      // falha da rota (sem sessão, sem permissão, chave da IA ausente ou sem
      // escopo no ambiente, IA fora do ar) vira uma frase curta e a tela segue
      // inteira nos controles manuais. Nunca vazar mensagem de erro da API
      // para o vendedor — "invalid x-api-key" não diz nada a ele.
      if (r.status === 422) {
        setErroFrase("Não entendi — tente reescrever.");
        return;
      }
      if (!r.ok || d.erro) {
        setAssistenteIndisponivel(true);
        setErroFrase(
          r.status === 401 || r.status === 403
            ? "Assistente indisponível — sessão sem permissão. Use os controles abaixo normalmente."
            : "Assistente indisponível neste ambiente. Use os controles abaixo normalmente.",
        );
        return;
      }
      ...
      // rede fora, resposta ilegível: mesma degradação, sem detalhe técnico
      setAssistenteIndisponivel(true);
      setErroFrase("Assistente indisponível neste ambiente. Use os controles abaixo normalmente.");
    } finally {
    ...
    >
      <div style={{ maxWidth: 1080, margin: "0 auto" }}>
        ...
        <div className="dsl-nao-imprime" style={{ marginTop: 18, background: "#0F1526", border: "1px solid #16213A", borderRadius: 14, padding: 14 }}>
          ...
          {erroFrase && (
            <p style={{ color: assistenteIndisponivel ? "#FCD34D" : "#F87171", fontSize: 12, margin: "8px 0 0" }}>
              {assistenteIndisponivel ? "⚠️ " : ""}
              {erroFrase}
            </p>
          )}
          {resumoPedido && (
