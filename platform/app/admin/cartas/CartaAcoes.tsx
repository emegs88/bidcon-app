"use client";
// Ação de status de carta no estoque global (admin). Client só para o clique:
// POST /api/admin/cartas/[id]/status → RPC definir_status_carta (atômica no servidor).
import { useState } from "react";
import { useRouter } from "next/navigation";
import { Button } from "@/components/ui/Button";
import { LABEL_STATUS_CARTA, type StatusCarta } from "@/lib/status";
import styles from "./acoes.module.css";

const OPCOES: StatusCarta[] = ["disponivel", "reservada", "indisponivel"];

export function CartaAcoes({
  cartaId,
  atual,
}: {
  cartaId: string;
  ...
  async function definir(novo: StatusCarta) {
    ...
    setErro(null);
    try {
      const res = await fetch(`/api/admin/cartas/${cartaId}/status`, {
        method: "POST",
